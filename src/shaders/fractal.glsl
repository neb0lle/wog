...
vec4 rayMarch(in vec3 from, in vec3 dir, in vec2 fragCoord) {
	...
	float totalDistance = Jitter * rand(fragCoord + vec2(uTime));
	vec3 dir2 = dir;
	float distance;
	int steps = 0;
	vec3 pos;
	...
	// 'AO' is based on the number of steps.
...
void main() {
	...
	vec3 camDir = normalize(target - camPos); // direction for the center ray
	camUp = normalize(camUp - dot(camDir, camUp) * camDir); // orthogonalize
	vec3 camRight = normalize(cross(camDir, camUp));

	vec2 coord = -1.0 + 2.0 * gl_FragCoord.xy / uResolution.xy;
